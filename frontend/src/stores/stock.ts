...
export const useStockStore = defineStore('stock', () => {
  ...
  async function fetchQuote(symbol: string, market = 'A') { loading.value = true; error.value = null; try { const res = await getQuote(symbol, market); if (res.code === 0 && res.data) currentQuote.value = res.data; } catch (e: any) { error.value = e.message; } finally { loading.value = false; } }
  async function fetchKline(symbol: string, market = 'A', startDate?: string, endDate?: string) { loading.value = true; error.value = null; try { const res = await getKline(symbol, market, startDate, endDate); if (res.code === 0 && res.data) klineData.value = res.data; } catch (e: any) { error.value = e.message; } finally { loading.value = false; } }
