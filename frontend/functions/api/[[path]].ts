export async function onRequest(context) {
  ...
  if (qm && method === "GET") {
    ...
    return new Response(JSON.stringify({code:0,message:"success",data:{
      symbol:sym,name:d.f58||sym,market:"A",
  ...
  // Stock search: GET /api/v1/stocks/search
  if (path === "/api/v1/stocks/search" && method === "GET") {
    ...
    const items = (r.data||{}).diff||[];
    const result = [];
    for(let i=0;i<items.length;i++){
      const c = String(items[i].f12||"");
      const n = String(items[i].f14||"");
      if(c.toLowerCase().includes(kw)||n.toLowerCase().includes(kw)){
        result.push({symbol:c,name:n,market:"A",exchange:c.startsWith("6")?"SSE":"SZSE",industry:""});
