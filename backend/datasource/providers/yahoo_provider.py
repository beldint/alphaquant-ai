...
class YahooStockProvider(StockProvider):
    ...
    async def get_realtime_quote(self, symbol: str, market: Market = "A") -> RealtimeQuote:
        ...
        vol = (quotes.get("volume") or [0])[-1] if isinstance(quotes.get("volume"), list) else 0
        return RealtimeQuote(
            symbol=symbol, name=meta.get("symbol", symbol),
            market=market, price=Decimal(str(reg)),
            change=Decimal(str(change)), pct_change=Decimal(str(round(pct, 2))),
            volume=Decimal(str(vol)), amount=Decimal(str(vol * reg)),
            timestamp=datetime.now(timezone.utc), source="yahoo")
        
    async def get_daily_kline(self, symbol: str, *, market: Market = "A",
        ...
        days = (end - start).days
        rng = "6mo" if days <= 180 else "1y" if days <= 365 else "2y" if days <= 730 else "5y" if days <= 1825 else "max"
        ...
        for i in range(len(timestamps)):
            if i < len(opens) and opens[i] is not None:
                ...
                v = v if v else 0
                bars.append(KlineBar(symbol=symbol, market=market, trade_date=td,
                    open_price=Decimal(str(o)), high_price=Decimal(str(h)),
                    low_price=Decimal(str(l)), close_price=Decimal(str(c)),
                    volume=Decimal(str(v)), amount=Decimal(str(v * c)), source="yahoo"))
        return bars
